...
export const MODES = ["Sentry Mode", "Silent Mode", "Teacher Mode"] as const;
...
export function ModesToolsPanel({
  ...
  onPlayMove: (m: { library: "emotions" | "dances"; id: string }) => void;
        ...
        >
          <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-border/60 bg-black/80">
            ...
            <div className="relative z-10 flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto p-3 md:p-4">
              <CollapsibleSection title="Modes" defaultOpen>
              ...
              <Separator className="bg-border/50" />
              <CollapsibleSection title="Moves & dances" defaultOpen={false}>
                <div className="grid grid-cols-5 gap-2">
                  {[...moveCatalog.emotions, ...moveCatalog.dances].map((m) => (
                    <Button
                      key={`${m.library}/${m.id}`}
                      type="button"
                      size="sm"
                      variant="outline"
                      className="h-11 px-0 py-0 text-center"
                      title={`${m.library}/${m.id}`}
                      onClick={() => onPlayMove({ library: m.library, id: m.id })}
                    >
